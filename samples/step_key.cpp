...
int main(int argc, char* argv[]) {
	...
	if (ifile) dfa.reset(DFA_Interface_load(ifile)); // by filename
	else       dfa.reset(DFA_Interface_load(stdin)); // by FILE*
	...
	for(int i = optind; i < argc; ++i) {
		...
		if (ok) {
			...
			on.text = text;
			on.pos  = int(ctx.pos);
			dfa->for_each_word(ctx.root, ctx.zidx, boost::ref(on));
		}
	}
	return 0;
}
